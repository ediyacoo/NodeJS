...
app.main={
	...
	initMap : function() {
		    ...
		    //init centerLonLat
		    app.initCenterLonLat=new OpenLayers.LonLat(55,24.3).transform(app.map.displayProjection, app.map.projection);
	...
	showLayer: function(obj, options){
			...
			if(obj.oplayer && obj.oplayer!='undefined'){
				...
				switch(obj.type){
					  ...
					  break;
					  case "WMS":
					  		var wms=new OpenLayers.Layer.WMS(obj.name, obj.url, obj.param, {isBaseLayer:false, visibility:true, singleTile:false, opacity:obj.opacity || 0.9});
						  	...
						  	wms.events.on({
						  		"loadend": function(){
							  		...
							  		if(!obj.timeDurations){obj.timeDurations=[]}
							  		obj.timeDurations.push(obj.timeDuration);
...
// 													
										...
										if(app.popup){
											...
											app.main.showDialog("attributes", "Identify", {width:300, height:450, modal:false, position:"left", resizable:false, close: function(){}});
										}
		...
		// 					
					  ...
					  case "TILEJSON":
							...
							obj.tileFeatures=[];
					  		...
					  		obj.oplayer=new OpenLayers.Layer.XYZ(obj.name, [obj.url], {attribution: "", isBaseLayer:false, sphericalMercator: true, transitionEffect: "resize", buffer: 1, numZoomLevels: 17, opacity:0.75,  getURL: function(bounds){
					  			//replace xyz
					  			var xyz=this.getXYZ(bounds);
	...
	showLayerslider: function(obj){
		...
		$("#layerslider").slider({
	        ...
	        change: function(e,ui){
	        	switch (obj.type){
	        		case "WMS":
	        			obj.param[obj.variable]=obj.values[ui.value];
	        		break;
	        		case "TMS":
	        			obj.url_new=obj.url+obj.values[ui.value] + "/";
	        		break;
	        		case "XYZ":
	...
	showContextMenu: function(dom,top){
		...
		var css={
			right:$("#layer").width()+10, //app.css.contextmenu_right,
			top: 0 || top,
			"z-index":3001
		}
		$(dom).css(css).show();
